...
class Recorder:
    def __init__(self, model, data, cams=("top","left_wrist","right_wrist"), fps=30, size=(320,240), out_root="datasets"):
        self.model = model
        self.data = data
        self.cams = list(cams)
        self.fps = float(fps)
...
def main():
    ...
    recorder = Recorder(model, data, cams=("top","left_wrist","right_wrist"), fps=30, size=(320,240))

    # ===== 新增：轨迹生成函数 =====
    def generate_trajectory(side, target_pos, target_quat, seconds=2.0, fps=15):
        """生成平滑的轨迹点"""
