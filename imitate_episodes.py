...
left_arm_move_j = None
right_arm_move_j = None
_ARM_WARNING_SHOWN = False

import IPython
e = IPython.embed
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") #lyu insert 20250708
...
def main(args):
    ...
    global left_arm_move_j, right_arm_move_j, _ARM_WARNING_SHOWN
    if is_sim:
        left_arm_move_j = None
        right_arm_move_j = None
        _ARM_WARNING_SHOWN = False
    ...
    if is_sim or task_name == 'all':
        ...
        if left_arm_move_j is None or right_arm_move_j is None:
            try:
                from arm import left_arm_move_j as _left_move, right_arm_move_j as _right_move
                left_arm_move_j = _left_move
                right_arm_move_j = _right_move
                _ARM_WARNING_SHOWN = False
            except Exception as exc:
                print(f"[warn] Failed to import arm controller: {exc}")
                left_arm_move_j = None
                right_arm_move_j = None
                _ARM_WARNING_SHOWN = False
    ...
    wandb.login(key="792d71d0d078f4dbacf8f7e51454b248b606f8c6")  # 或使用wandb.login(relogin=True)强制重新登录:ml-citation{ref="3" data="citationList"}
...
def get_image(ts, camera_names, rand_crop_resize=False):
    ...
    curr_image = torch.from_numpy(curr_image / 255.0).float().to(device).unsqueeze(0) #curr_image = torch.from_numpy(curr_image / 255.0).float().cuda().unsqueeze(0)
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    loading_status = policy.deserialize(torch.load(ckpt_path)) # torch.load(..., map_location=torch.device("cpu"))
    ...
    if vq:
        ...
        latent_model.deserialize(torch.load(latent_model_ckpt_path)) # torch.load(..., map_location=torch.device("cpu"))
        latent_model.eval()
        latent_model.to(device) #latent_model.cuda()
    ...
    for rollout_id in range(num_rollouts):
        ...
        if 'sim_transfer_cube' in task_name:
            BOX_POSE[0] = [-0.3, -0.7, 0.57, 1, 0, 0, 0] # sample_box_pose() # used in sim reset
        ...
        ### evaluation loop
        if temporal_agg:
            all_time_actions = torch.zeros([max_timesteps, max_timesteps+num_queries, 16]).to(device)
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                qpos = torch.from_numpy(qpos).float().to(device).unsqueeze(0) #qpos = torch.from_numpy(qpos).float().cuda().unsqueeze(0)
                # qpos_history[:, t] = qpos
                ...
                # ↓ Show joints data
                # print(
                #     f"[eval_bc] rollout {rollout_id} step {t} target_qpos: "
                #     f"{np.array2string(target_qpos, precision=5, separator=', ')}"
                # )
                # ↑ Show joints data

                if real_robot:
                    global _ARM_WARNING_SHOWN
                    if left_arm_move_j is not None and right_arm_move_j is not None:
                        left_arm_move_j(target_qpos[:7])
                        right_arm_move_j(target_qpos[7:])
                    elif not _ARM_WARNING_SHOWN:
                        print("[warn] real_robot=True but arm controller unavailable; skip joint command.")
                        _ARM_WARNING_SHOWN = True
        ...
        valid_rewards = rewards[rewards!=None]
        episode_highest_reward = np.max(valid_rewards) if len(valid_rewards) > 0 else 0
...
def forward_pass(data, policy, device): #(data, policy):
